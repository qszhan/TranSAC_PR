...
def return_nclass_data_semi(labels, features, probs, nclasses, ratio):
    """Only return the ratio% of example ids from the top-n classes."""
    # nclass_labels = []
    # nclass_features = []
    # nclass_probs = []
    selected_indices = []
    for i in range(nclasses):
        ith_class_index = select_class_index(labels, i)  # 600
        # Select ratio% of the indices from ith_class_index
        # train_indices = []
        selected_indices += random.sample(ith_class_index, k=int(ratio * len(ith_class_index)))

    # Create a new dataset with only the selected samples
    nclass_features = np.take(np.array(features), selected_indices, axis=0)
    nclass_labels = np.take(np.array(labels), selected_indices, axis=0)
    nclass_probs = np.take(np.array(probs), selected_indices, axis=0)
    return nclass_labels, nclass_features, nclass_probs


def select_class_index(label, cla):
    # generate a list to save the instance ID of minority class
    index = []
    # print('cla', cla)
